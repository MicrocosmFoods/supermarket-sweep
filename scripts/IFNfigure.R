...
# -----------------------------------------------------------------------------
# plot constants - category order and colors
# -----------------------------------------------------------------------------
...
# -----------------------------------------------------------------------------
# create subset for the control types
# -----------------------------------------------------------------------------
...
# -----------------------------------------------------------------------------
# join with metadata and map categories for unfiltered IFN by viability
# -----------------------------------------------------------------------------
...
# -----------------------------------------------------------------------------
# join with metadata and map categories for filtering results by viability > 70%
# -----------------------------------------------------------------------------
...
# violin plots
# -----------------------------------------------------------------------------
unfiltered_ifn_violin_plot <- ggplot(unfiltered_plot_df, aes(x = Group, y = normalized_IFN, fill = Group, color = Group)) +
...
filtered_ifn_violin_plot <- ggplot(filtered_plot_df, aes(x = Group, y = normalized_IFN, fill = Group, color = Group)) +
  geom_violin(alpha = 0.5, scale = "width", trim = FALSE, linewidth = 0.8) +
  geom_boxplot(width = 0.1, fill = "white", color = "gray20", outlier.shape = NA, alpha = 0.8) +
  geom_jitter(width = 0.15, size = 1.5, alpha = 0.7) +
  geom_hline(yintercept = 100, linetype = "dashed", color = "gray50", linewidth = 0.6) +
